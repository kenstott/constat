...
class NerScopeCacheModel(Base):
    __tablename__ = "ner_scope_cache"

    fingerprint = Column(Text, primary_key=True)
    created_at = Column(TIMESTAMP)
    entity_count = Column(Integer, default=0)


class NerCachedEntityModel(Base):
    __tablename__ = "ner_cached_entities"

    fingerprint = Column(Text, nullable=False, primary_key=True)
...
class NerCachedChunkEntityModel(Base):
    __tablename__ = "ner_cached_chunk_entities"

    fingerprint = Column(Text, nullable=False, primary_key=True)
    chunk_id = Column(Text, nullable=False, primary_key=True)
    entity_id = Column(Text, nullable=False, primary_key=True)
    confidence = Column(Float, default=1.0)


class NerCachedClusterModel(Base):
    __tablename__ = "ner_cached_clusters"

    fingerprint = Column(Text, nullable=False, primary_key=True)
    term_name = Column(Text, nullable=False, primary_key=True)
    cluster_id = Column(Integer, nullable=False)
