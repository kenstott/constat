...
# Canary: 123595a5-f68b-4f25-b779-4df06970d73e
#
...
def _load_personal_accounts(
    session_manager: SessionManager,
    managed: ManagedSession,
    user_id: str,
) -> None:
    """Load personal accounts (IMAP, email, etc.) after SESSION_READY."""
    ...
    try:
        ...
        server_config = managed.session.config
        personal_accounts = load_user_accounts(
            user_id,
            getattr(server_config, 'data_dir', None),
        )
        loaded_any = False
        for acct_name, acct in personal_accounts.items():
            if not acct.active or not managed.session.doc_tools:
                continue
            session_manager._push_event(managed, EventType.SOURCE_INGEST_START, {"name": acct_name})
            try:
                doc_kwargs = account_to_document_config(acct, server_config)
                doc_config = DocumentConfig(**doc_kwargs)
                success, msg = managed.session.doc_tools.add_document_from_config(
                    acct_name, doc_config,
                    domain_id=user_id,
                    session_id=managed.session_id,
                )
                if success:
                    ...
                    session_manager._push_event(managed, EventType.SOURCE_INGEST_ERROR, {"name": acct_name, "error": msg})
            except Exception as e:
                logger.warning(f"Failed to load personal account {acct_name}: {e}")
...
async def create_session(
    ...
    def _init_session():
        ...
        try:
            ...
            # Load personal accounts after SESSION_READY (may take time for IMAP)
            threading.Thread(
                target=_load_personal_accounts,
                args=(session_manager, _managed, effective_user_id),
                name=f"acct-load-{client_session_id[:8]}",
                daemon=True,
            ).start()
        except Exception as e:
