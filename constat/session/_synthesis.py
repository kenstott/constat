...
# Canary: 27169559-6322-41d9-b499-9e65fce3935d
#
...
class SynthesisMixin:
    ...
    def _synthesize_answer(self, problem: str, step_outputs: str, artifacts: list[dict] | None = None) -> str:
        ...
        # Truncate step outputs to avoid exceeding context limits
        max_output_chars = 8000
        truncated_outputs = step_outputs
        if len(step_outputs) > max_output_chars:
            truncated_outputs = step_outputs[:max_output_chars] + "\n\n[... truncated for brevity ...]"
...
Reference tables with backticks: `table_name`"""
        ...
        if not result.success:
            logger.warning(f"Answer synthesis failed ({result.provider_used}/{result.model_used}): {result.content}")
            return "Analysis completed but answer synthesis failed. See step outputs above."
        logger.info(f"[Synthesis] Generated by {result.provider_used}/{result.model_used}")
