...
# Canary: 138a376a-abb7-4a77-833a-772e9d1da24f
#
...
class ExecutionMixin:
    ...
    def _execute_step(self, step: Step) -> StepResult:
        ...
        # Less capable models (first in chain) get fewer retries before escalating.
        # First model gets 2 attempts; last model gets the remainder of the budget.
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            # Emit escalation events so the UI shows failed model attempts
            for esc in result.escalations:
                self._emit_event(StepEvent(
                    event_type="model_escalation",
                    step_number=step.number,
                    data={
                        "from_model": esc.from_model,
                        "to_model": esc.to_model,
                        "reason": esc.reason,
                        "attempt": attempt,
                    }
                ))
            ...
            if result.model_index > skip_models:
                skip_models = result.model_index
            ...
            self._emit_event(StepEvent(
                ...
                data={
                    ...
                    "model": codegen_model,
                    "escalations": [
                        {"from": esc.from_model, "to": esc.to_model, "reason": esc.reason}
                        for esc in result.escalations
                    ] if result.escalations else None,
                }
            ))
                ...
                if step.post_validations:
                    ...
                    if failed_validation:
                        if failed_validation.on_fail == ValidationOnFail.CLARIFY:
                            ...
                            # Count validation failures toward model escalation
                            runtime_failures += 1
                            if runtime_failures >= escalation_threshold:
                                skip_models += 1
                                runtime_failures = 0
                                logger.info(f"[Step {step.number}] Escalating after {escalation_threshold} validation failures, skip_models={skip_models}")
                            continue
            ...
            if runtime_failures >= escalation_threshold:
                skip_models += 1
                runtime_failures = 0
                logger.info(f"[Step {step.number}] Escalating after {escalation_threshold} runtime failures, skip_models={skip_models}")
    ...
    def _run_post_validations(
        ...
        for v in step.post_validations:
            ...
            try:
                ...
                passed = False
                # List available tables/variables so LLM can fix the name mismatch
                store_tables = [t["name"] for t in self.datastore.list_tables()] if self.datastore else []
                user_vars = [k for k in namespace if not k.startswith("_")]
                diagnostic = (
                    f"Expression raised {type(e).__name__}: {e}\n"
                    f"Available store tables: {store_tables}\n"
                    f"Available variables: {user_vars}\n"
                    f"The code must save data using the exact name expected by the validation expression."
                )
            except Exception as e:
                logger.warning(f"[Step {step.number}] Post-validation expression error: {v.expression} -> {e}")
                passed = False
